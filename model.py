...
def scaled_dot_product_attention(
    ...
    K_T = K.transpose(-2, -1)
    num = torch.matmul(Q, K_T)

    denom = math.sqrt(Q.size(-1))
    res = num / denom
    if mask is not None:
        res = res.masked_fill(mask, float('-inf'))
    
    attn_w = F.softmax(res, dim=-1)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        value_proj = self.W_V(value)

        batch_size = query.size(0)

        query_proj = query_proj.view(batch_size, -1, self.num_heads, self.d_k).transpose(1, 2)
...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        if checkpoint_path is not None:
            gdown.download(id="1vBoBzcSt-KENqLnx24xh4IFi0z4kcTKO", output=checkpoint_path, quiet=False)
    ...
    def encode(
        ...
        src_embedded = self.pos_enc(self.src_embed(src))
    ...
    def decode(
        ...
        tgt_embedded = self.pos_enc(self.tgt_embed(tgt))
